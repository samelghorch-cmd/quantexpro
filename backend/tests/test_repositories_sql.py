...
UTC = dt.UTC
TS = dt.datetime(2026, 7, 24, tzinfo=UTC)
...
async def test_upsert_bars_is_idempotent_sql() -> None:
    ...
    bar = BarIn(symbol="BTC", ts=TS, open=1, high=2, low=1, close=1.5, volume=10)
...
async def test_upsert_ticks_conflict_on_trade_id() -> None:
    session = _CapturingSession()
    tick = TickIn(symbol="BTC", ts=TS, trade_id="t1", price=100, size=1)
...
async def test_upsert_orderbook_conflict() -> None:
    session = _CapturingSession()
    ob = OrderbookL2In(symbol="BTC", ts=TS, bids=[[100, 1]], asks=[[101, 1]])
