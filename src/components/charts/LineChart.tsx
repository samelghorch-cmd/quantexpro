...
export function LineChart({
  ...
  const all = series.flatMap((s) => s.data.filter((v): v is number => v != null && !Number.isNaN(v)));
  ...
  const sx = (i: number) => padL + (i / Math.max(1, maxLen - 1)) * (W - padL - pad);
  const sy = (v: number) => pad + (H - pad - padB) * (1 - (v - lo) / (hi - lo));
  const fmtY = yFormat || ((v: number) => (Math.abs(v) >= 1000 ? (v / 1000).toFixed(0) + "k" : v.toFixed(1)));
