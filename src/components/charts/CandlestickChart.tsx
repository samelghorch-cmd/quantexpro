// Chandeliers Canvas + overlays + flèches signaux.
...
export interface ChartSignal {
  data: Array<boolean | number | null | undefined>;
...
export function CandlestickChart({
  ...
  useEffect(() => {
    ...
    overlays.forEach((ov) => {
      for (let i = start; i < end; i++) {
        const v = ov.data[i];
        if (v != null && !isNaN(v)) { if (v > hi) hi = v; if (v < lo) lo = v; }
      }
    });
    const pad = (hi - lo) * 0.05;
    hi += pad; lo -= pad;
    const yScale = (v: number) => padT + chartH - (v - lo) / (hi - lo) * chartH;
  ...
  }, [bars, overlays, signals, width, height, view]);
