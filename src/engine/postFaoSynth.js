...
function stabilityScore(equityCurve, capital, K = 6) {
...
function robustnessScore(bars, ctx, strategy, params, baseSharpe) {
  const perturbs = [0.9, 1.1];
  const results = [];
  for (const f of perturbs) {
    const p = { ...params, slAtr: Math.max(0.5, params.slAtr * f), tpAtr: params.tpAtr > 0 ? params.tpAtr * f : 0 };
...
function withRegimeInline(evalFn, regime) {
...
export function runPostFAO(faoResult, bars, ctx, strategy, capital = 100000) {
  const combos = faoResult.combos.slice(0, 50); // top survivants pour la robustesse
  if (combos.length === 0) return { ranked: [], best: null };
