// P4-SIGNAL-WS — logique console Signal Engine (slots + consensus + ring + WS bars).
// Éval stratégies = pure (même moteur que backtest). Le WS backend alimente le journal ;
// le mode local réévalue sur le ctx pipeline.

export const DEFAULT_SIGNAL_SLOTS = [1, 3, 4, 21, 31, 55];
export const MAX_SIGNAL_EVENTS = 200;

/**
 * URL WebSocket `/stream/bars/{tf}?api_key=`
 * @param {string} baseUrl  http(s) API
 * @param {string} timeframe  ex. 15m
 * @param {string} [apiKey]
 */
export function barsWebSocketUrl(baseUrl, timeframe, apiKey = "") {
  const base = String(baseUrl || "").replace(/\/$/, "");
  if (!base) throw new Error("URL API manquante");
  const u = new URL(base);
  u.protocol = u.protocol === "https:" ? "wss:" : "ws:";
  u.pathname = `/stream/bars/${encodeURIComponent(timeframe)}`;
  u.search = "";
  if (apiKey) u.searchParams.set("api_key", apiKey);
  return u.toString();
}

/** Map TF dashboard (facteur barres) → timeframe API stream. */
export const TF_FACTOR_TO_API = {
  1: "5m",
  3: "15m",
  12: "1h",
  48: "4h",
  288: "1d",
};

export function resolveStreamTf(tfFactor, fallback = "15m") {
  return TF_FACTOR_TO_API[Number(tfFactor)] || fallback;
}

/**
 * Parse message texte WS (JSON EventEnvelope).
 * @returns {{ id: string, type: string, ts: string|null, symbol: string|null, timeframe: string|null, bar: object|null, raw: object } | null}
 */
export function parseBarClosedMessage(raw) {
  let obj;
  try {
    obj = typeof raw === "string" ? JSON.parse(raw) : raw;
  } catch {
    return null;
  }
  if (!obj || typeof obj !== "object") return null;
  const type = String(obj.type || "");
  const payload = obj.payload && typeof obj.payload === "object" ? obj.payload : {};
  return {
    id: String(obj.id || `${payload.symbol || "?"}:${payload.ts || Date.now()}`),
    type: type || "unknown",
    ts: obj.ts != null ? String(obj.ts) : payload.ts != null ? String(payload.ts) : null,
    symbol: payload.symbol != null ? String(payload.symbol) : null,
    timeframe: payload.timeframe != null ? String(payload.timeframe) : null,
    bar: payload,
    raw: obj,
  };
}

/**
 * Évalue une liste de slots stratégie sur l'index de barre `i`.
 * @param {Array<{ id: number, name: string, cat?: string, eval: Function }>} library
 * @param {object} ctx
 * @param {number} i
 * @param {number[]} [slotIds]
 */
export function evaluateSlots(library, ctx, i, slotIds = DEFAULT_SIGNAL_SLOTS) {
  const lib = library || [];
  const byId = new Map(lib.map((s) => [s.id, s]));
  return (slotIds || []).map((id) => {
    const s = byId.get(id);
    if (!s) {
      return { id, name: `#${id} ?`, cat: "?", long: false, short: false, missing: true };
    }
    let sig = { long: false, short: false };
    try {
      if (ctx && i >= 1) sig = s.eval(ctx, i) || sig;
    } catch {
      /* stratégie cassée → flat */
    }
    return {
      id,
      name: s.name,
      cat: s.cat || "?",
      long: Boolean(sig.long),
      short: Boolean(sig.short),
      missing: false,
    };
  });
}

export function consensusFromSignals(signals) {
  const list = signals || [];
  const nLong = list.filter((s) => s.long).length;
  const nShort = list.filter((s) => s.short).length;
  const consensus = nLong > nShort ? "LONG" : nShort > nLong ? "SHORT" : "NEUTRE";
  return { consensus, nLong, nShort, nFlat: list.length - nLong - nShort, nSlots: list.length };
}

/**
 * Ajoute un événement en tête (ring borné).
 * @template T
 * @param {T[]} ring
 * @param {T} event
 * @param {number} [max]
 */
export function pushRing(ring, event, max = MAX_SIGNAL_EVENTS) {
  const next = [event, ...(ring || [])];
  if (next.length > max) next.length = max;
  return next;
}

/** Événement console à partir d'un snapshot slots. */
export function makeSignalSnapshotEvent(signals, meta = {}) {
  const c = consensusFromSignals(signals);
  return {
    id: `sig-${meta.ts || Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
    kind: "signal_snapshot",
    ts: meta.ts || Date.now(),
    source: meta.source || "local",
    symbol: meta.symbol || null,
    consensus: c.consensus,
    nLong: c.nLong,
    nShort: c.nShort,
    signals: (signals || []).filter((s) => s.long || s.short).map((s) => ({
      id: s.id,
      name: s.name,
      side: s.long ? "LONG" : "SHORT",
    })),
  };
}

/** Événement console à partir d'un bar.closed WS. */
export function makeBarClosedEvent(parsed, meta = {}) {
  return {
    id: `bar-${parsed.id}`,
    kind: "bar_closed",
    ts: parsed.ts ? Date.parse(parsed.ts) || Date.now() : Date.now(),
    source: meta.source || "ws",
    symbol: parsed.symbol,
    timeframe: parsed.timeframe,
    eventId: parsed.id,
    close: parsed.bar?.close ?? parsed.bar?.c ?? null,
  };
}

export function filterEvents(events, { kind = "", q = "" } = {}) {
  const kk = String(kind || "").trim();
  const qq = String(q || "").trim().toLowerCase();
  return (events || []).filter((e) => {
    if (kk && e.kind !== kk) return false;
    if (!qq) return true;
    const hay = `${e.kind} ${e.symbol || ""} ${e.consensus || ""} ${e.eventId || ""}`.toLowerCase();
    return hay.includes(qq);
  });
}

export function eventsToCsv(events) {
  const header = ["ts", "kind", "source", "symbol", "consensus", "nLong", "nShort", "close", "eventId"];
  const lines = [header.join(",")];
  for (const e of events || []) {
    const cells = [
      e.ts ? new Date(e.ts).toISOString() : "",
      e.kind,
      e.source,
      e.symbol,
      e.consensus,
      e.nLong,
      e.nShort,
      e.close,
      e.eventId,
    ].map((v) => {
      const s = v == null ? "" : String(v);
      return /[",\n]/.test(s) ? `"${s.replace(/"/g, '""')}"` : s;
    });
    lines.push(cells.join(","));
  }
  return lines.join("\n");
}
