// TCA — Transaction Cost Analysis (P1-TCA).
...
export function classIdForAsset(assetKey) {
  return REAL_ASSET_CLASS[assetKey] || "synthetic";
}

/** Coût modèle one-way / round-trip en bps (fraction × 10 000). */
export function modelCostBps(classId = "synthetic") {
  const m = COST_MODELS[classId] || COST_MODELS.synthetic;
  const oneWay = ((m.feePct || 0) + (m.spreadPct || 0)) * 1e4;
  return {
    classId: COST_MODELS[classId] ? classId : "synthetic",
    label: m.label,
    feePct: m.feePct || 0,
 ...
 * side: +1 long, −1 short.
 */
export function slipBps(signalPrice, fillPrice, side = 1) {
...
export function slipUsd(signalPrice, fillPrice, side, notional) {
 ...
 */
export function fillsFromTrades(trades, bars, { notional = 100000 } = {}) {
  ...
  for (const t of trades) {
    const side = t.side === -1 || t.side === "short" ? -1 : 1;
    ...
    const notion = Number.isFinite(t.notional) ? t.notional
      : (Number.isFinite(t.ret) && t.ret !== 0 && Number.isFinite(t.pnl)
        ? Math.abs(t.pnl / t.ret)
        : notional);
    const entryBps = slipBps(signal, fill, side);

    // Exit : signal = prix théorique enregistré ; fill ≈ close de la barre de sortie (proxy).
    let exitSignal = Number(t.exit);
...
/**
 * Analyse TCA agrégée.
 * @param {Array} fills — fills (entrySlipBps déjà calculé ou signal/fill/side)
 * @param {{ classId?: string, assetKey?: string, price?: number }} opts
 */
export function runTCA(fills, opts = {}) {
  const list = (fills || []).map((f) => {
    if (Number.isFinite(f.entrySlipBps)) return f;
    const side = f.side === -1 || f.side === "short" ? -1 : 1;
    const entrySlipBps = slipBps(f.signalPrice, f.fillPrice, side);
    return {
      ...f,
      side,
      entrySlipBps,
      isUsd: slipUsd(f.signalPrice, f.fillPrice, side, f.notional || opts.notional || 100000),
    };
  ...
  const rtSlips = list.map((f) => (Number.isFinite(f.roundTripSlipBps) ? f.roundTripSlipBps : f.entrySlipBps * 2));
  ...
  const baseSpread = model.spreadPct ?? model.model?.spreadPct ?? 0;
