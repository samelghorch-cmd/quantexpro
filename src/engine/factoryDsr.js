// Garde-fou DSR pour l'Usine à Stratégies (P1).
...
export function trialsForFactoryPair(screenedCount, gridSize) {
 ...
 * @returns {{ dsr:number, sr:number, srStar:number, sigmaSR:number, nTrials:number }}
 */
export function evaluateFactoryDsr(trades, nTrials) {
  const pnls = (trades || []).map((t) => t.pnl).filter((x) => Number.isFinite(x));
  return deflatedSharpe(pnls, nTrials);
}
 ...
 */
export function passesFactoryDsr(dsrResult, { oos = true, minDsr = FACTORY_DSR_MIN } = {}) {
  if (!oos) return true;
